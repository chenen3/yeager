...
func startTunnel(obfs bool) (*TunnelServer, *TunnelClient, error) {
	...
	ts := new(TunnelServer)
	go func() {
		if e := ts.Serve(lis.Addr().String(), srvTLSConf, obfs); e != nil {
			log.Print(e)
		}
	}()
	...
	tc := NewTunnelClient(lis.Addr().String(), cliTLSConf, obfs)
	return ts, tc, nil
}
...
func BenchmarkThroughputWithObfs(b *testing.B) {
	echo, err := ynet.StartEchoServer()
	if err != nil {
		b.Fatal(err)
	}
